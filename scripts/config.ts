...
export function loadConfig(envFilePath: string = defaultEnvFile()): AppConfig {
  ...
  return {
    ...
    gatewayPrivateKey: required(values, "GW_PRIVATE_KEY"),
    ...
    gatewayEncryptionMode: (values.GW_ENCRYPTION_MODE as EncryptionMode) || "optional",
    clientPrivateKey: required(values, "CVM_CLIENT_PRIVATE_KEY"),
    serverPubkey: required(values, "CVM_SERVER_PUBKEY"),
    proxyEncryptionMode: (values.CVM_PROXY_ENCRYPTION_MODE as EncryptionMode) || "optional",
    ...
    responseLookbackSeconds: parseNumber(values.CVM_RESPONSE_LOOKBACK_SECONDS, 300),
    browserHost: values.CSH_BROWSER_HOST || "127.0.0.1",
    browserPort: parseNumber(values.CSH_BROWSER_PORT, 4318),
    browserAllowRemote: parseBoolean(values.CSH_BROWSER_ALLOW_REMOTE, false),
    sessionIdleTtlSeconds: parseNumber(values.CSH_SESSION_IDLE_TTL_SECONDS, 1800),
    closedSessionTtlSeconds: parseNumber(values.CSH_CLOSED_SESSION_TTL_SECONDS, 300),
  };
}
...
export function validateConfig(config: AppConfig, mode: "host" | "client" | "full" = "full"): ConfigCheckResult {
  ...
  if ((fileStat.mode & 0o077) !== 0) {
    warnings.push(`${config.envFilePath} is readable by group or others; chmod 600 is recommended.`);
  }
...
export function writeBootstrapEnv(outputFile: string): { outputFile: string; allowedClientPubkey: string } {
  ...
  const text =
    ...
    formatEnvLine("CVM_RELAYS", "wss://relay.contextvm.org") +
...
Type=simple
User=${user}
${groupLine}WorkingDirectory=${rootDir}
ExecStart=${path.join(rootDir, "bin", "csh")} host start ${configPath}
Restart=always
RestartSec=3
